...
\begin{equations}
	...
	& = \bvd{x}[k] X_1[l,k] + \bvk[l,k]
\end{equations}
where $\bvp{B,l'}[k]$ is the $l'$-th column of $\bvB[k]$. With this, $\bvd{x}[k] X_1[l,k]$ is the desired speech component of $\bvB[k] \bvx{1}[l,k]$ with $\bvd{x}[k]$ being the desired-speech frequency response; and $\bvk[l,k]$ (the summation over $l \neq 0$) is the undesired component, or reverberation signal. 
...
\begin{equation}
	...
	\bvw[l,k] = \bvk[l,k] + \bvC[k] \bvv{1}[l,k] + \bvr[l,k]
\end{equation}
and therefore
\begin{equation}
...
\begin{subgather}{eqs:window-averaged_narrowband_metrics}
	...
	\gsrr[k] = \frac{\sum_{l} \var{X_1}[l,k] \abs{\he{\bvf}[l,k] \bvd{x}[k]}^2}{\sum_{l} \he{\bvf}[l,k] \Corr{\bvk}[l,k] \bvf[l,k] } \div \frac{\sum_{l} \var{X_1}[l,k]}{\sum_{l} \var{\kappa_1}[l,k]} \\
...
\begin{subgather}{eqs:window-averaged_broadband_metrics}
	...
	\gsrr = \frac{\sum\limits_{l,k} \var{X_1}[l,k] \abs{\he{\bvf}[l,k] \bvd{x}[k]}^2}{\sum\limits_{l,k} \he{\bvf}[l,k] \Corr{\bvk}[l,k] \bvf[l,k] } \div \frac{\sum\limits_{l,k} \var{X_1}[l,k]}{\sum\limits_{l,k} \var{\kappa_1}[l,k]} \\
	\dsrf = \frac{\sum\limits_{l,k} \var{X_1}[l,k]}{\sum\limits_{l,k} \var{X_1}[l,k] \abs{\he{\bvf}[l,k] \bvd{x}[k]}^2}
\end{subgather}
where $\kappa_1[l,k]$ is the first element (corresponding to the reference sensor) of $\bvk[l,k]$. It is easy to see that if $\dsrf[l,k] = 1$ then the distortionless constraint is satisfied. Also, $\dsrf[l,k] = 1$ implies that $\dsrf[k] = \dsrf = 1$.