...
From \cref{subfig:lineplot_dsrf_32_n15_acc}, we see that all beamformers had a negligible distortion of less than $0.5\dB$, however the distortion for the STFT and \tssbt{} beamformers was still closer to zero.

From the ERLE results in \cref{subfig:lineplot_erle_32_n15_acc} it is noticeable that the STFT and \nssbt{} beamformers had a similar performance, with the \tssbt{} one being slightly better, both outperforming the \nssbt{} beamformer's results by approx. $4\dB$. A similar result can be seen in \cref{subfig:lineplot_nsrf_32_n15_acc} for the NSRF, however in this regard the STFT had a minimal margin over the \tssbt{}, and their gap compared to the \nssbt{} wasn't as prominent, although still visible.

We will also compare the results with a varying input SERs, to assess the beamformer's performances for different loudspeaker signal levels. For such, we will use the time-average metrics as in \cref{eq:sec3:time-average_metrics}. The other parameters and variables are maintained from the previous simulations, with only the SER being changed.

\input{figures/io_input/aux_data_32.tex}
\begin{figure}[H]
	\begin{subfigure}{\textwidth}
		\centering
		\input{figures/lineplot_dsrf_32_avg_acc.tex}
		\caption{Time-average broadband DSRF.}
		\label{subfig:lineplot_dsrf_32_avg_acc}
	\end{subfigure}\\[1em]
	\begin{subfigure}{\textwidth}
		\centering
		\input{figures/lineplot_erle_32_avg_acc.tex}
		\caption{Time-average broadband ERLE.}
		\label{subfig:lineplot_erle_32_avg_acc}
	\end{subfigure}\\[1em]
	\begin{subfigure}{\textwidth}
		\centering
		\input{figures/lineplot_nsrf_32_avg_acc.tex}
		\caption{Time-average broadband NSRF.}
		\label{subfig:lineplot_nsrf_32_avg_acc}
	\end{subfigure}\\[1em]
	\centering\ref*{dsrf_lineplot_32_n15_acc}
	\caption{Output metrics for the beamformers.}
	\label{fig:lineplots_32_avg_acc}
\end{figure}

As seen in \cref{subfig:lineplot_dsrf_32_avg_acc}, the SF-SSBT beamformer caused some distortion on the desired signal. However, this distortion is minimal, and decreases as the loudspeaker SNR increases.
