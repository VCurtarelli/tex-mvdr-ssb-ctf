...
%\end{changingfrom}
This isn't as straight-forward with the RFT, since after the convolution each frequency depends on its conjugate as well. We consider each system to have two inputs $X'(f) = X(f)$ and $X''(f)= X(\-f)$ and two frequency responses $H_m'(f)$ and $H_m''(f)$ (where $m$ represents the system's index). Then, our outputs can be described as
...
\end{equation}
in which this convolution is done over the frames $l$, with $H_{\sS}'[l,k] = \Re{H}_{\sF}[l,k]$ and $H_{\sS}''[l,k] = \Im{H}_{\sF}[l,k]$. Note that this is an approximation, as for the real convolution theorem one would need to consider cross-band interference \cite{avargel_system_2007}, however this effect wasn't considered here, neither for the STFT nor the SSBT. Also note that the presence of the conjugate frequency is a byproduct different to the cross-band interference, which happens due to aliasing.
