...
%\subsection{Filtering with regularization}
%
%We now change our matrix to ....
%With this, our filter will having a small tradeoff between minimization of the undesired signal and WNG, through the parameter $\epsilon$.
%Maybe move this to a new subsection 3.2 after 3.1, and explain the robust MPDR with regularization. Consider for the first sims here (4.1 to 4.5) that $\epsilon = 0$, and in this last one $\epsilon \neq 0$, and fare the results. Compare different $\Ly$ with $\epsilon = 0$ and $\epsilon = ??$? Still to do, needs more thought, needs implementation.
