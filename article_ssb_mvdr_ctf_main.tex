...
\Title{{\smaller[4][DRAFT]} On the Single-Sideband Transform for MVDR Beamformers}
...
\abstract{%
This paper investigates the application of the Single-Sideband Transform (SSBT) for constructing a Minimum-Variance Distortionless-Response (MVDR) beamformer in the context of the convolutive transfer function (CTF) model for short-windows time-frequency transforms. Our study aims to optimize the utilization of SSBT in this endeavor, by examining its characteristics and traits. We address a reverberant scenario with multiple noise sources, aiming to minimize both undesired interference and reverberation in the output. Through simulations reflecting real-life scenarios, we show that employing the SSBT, both in a naive and a refined approach, results in superior signal enhancement compared to the Short-Time Fourier Transform (STFT). The refined approach not only enhances the signal but also ensures the desired distortionless behavior, which is not achieved by the naive one.
}
...
%
\input{parts/4.True_MVDR_SSBT.tex}
%
